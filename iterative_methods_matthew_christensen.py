# -*- coding: utf-8 -*-
"""Iterative_methods-Matthew_Christensen.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/11Fp6vwRDZnYrB3H9PTQsa1-OAy9bmWBp
...
def jacobi1_iteration(x,y): 
    ...
    my_list = [new_x, new_y]
    return my_list # Your return statement should return a list of the form [new_x,new_y]
...
def jacobi1_method(n):
    ...
    while(n > 0):
        my_list = jacobi1_iteration(x_n, y_n)
        x_n = my_list[0]
        y_n = my_list[1]
...
def gs1_error(n): 
    real_list = [x_val, y_val]
    est_list = gs1_method(n)
    
    return np.linalg.norm(np.array(real_list) - np.array(est_list))
...
def gs3_iteration(x,y,z): 
    new_x = (-8 + 2*y - 3*z)*1/5
    new_y = (102 - new_x + 4*z)*1/4
    new_z = (-90 + 2*new_x + 2*new_y)*1/4
    
    my_list = [new_x, new_y, new_z]
...
def gs3_method(n): 
    ...
    while(n>0):
        my_list = gs3_iteration(x_n, y_n, z_n)
        x_n = my_list[0]
        y_n = my_list[1]
        z_n = my_list[2]
        n -= 1
